...
@section('content')
    <x-admin.page-header :title="'Data ' . $page['title']" subtitle="Kelola hari libur nasional">
        <div class="flex gap-2">
            <x-admin.button href="{{ route('admin.holidays.add') }}" class="font-bold">
    ...
    <div class="mb-6">
        <form action="{{ route('admin.holidays.index') }}" method="GET" navigate-form
            class="flex flex-col sm:flex-row items-center gap-3">
            <div class="w-full sm:w-64">
                <x-admin.input :label="null" name="keywords" :value="$keywords ?? ''" placeholder="Nama hari libur" size="sm" />
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                @if (!empty($keywords) || !empty($month) || !empty($year))
                    <x-admin.button href="{{ route('admin.holidays.index') }}" size="sm" color="outline-secondary">
        ...
        <x-admin.table>
            <x-admin.table.thead>
                <tr>
                    ...
                    <x-admin.table.th>Nama Hari Libur</x-admin.table.th>
                    ...
                    <x-admin.table.tr>
                        <x-admin.table.td>
                            <span class="text-sm font-medium text-gray-800 dark:text-neutral-200">
                                {{ $d->holiday_date ? \Carbon\Carbon::parse($d->holiday_date)->translatedFormat('l, d F Y') : '-' }}
                            </span>
                        </x-admin.table.td>
                        <x-admin.table.td class="!whitespace-normal min-w-[12rem]">
                            <span class="text-sm font-semibold text-gray-800 dark:text-neutral-200">{{ $d->holiday_name }}</span>
                        </x-admin.table.td>
                        <x-admin.table.td innerClass="px-6 py-1.5 flex items-center justify-end gap-x-1">
                            <a navigate
                                ...
                                href="{{ route('admin.holidays.update', $d->id) }}" title="Edit">
                            ...
                            <button type="button"
                                ...
                                onclick="setDeleteData('{{ $d->id }}', '{{ e($d->holiday_name) }}')">
            ...
            <div
                ...
                <div class="p-4 sm:p-10 text-center overflow-y-auto">
                    ...
                    <p class="text-gray-500 dark:text-neutral-500">
                        Apakah Anda yakin ingin menghapus <span id="delete-item-title"
    ...
    <script>
        function setDeleteData(id, title) {
            ...
            document.getElementById('delete-form').action = `/admin/holidays/delete/${id}`;
        }
    </script>
@endsection
